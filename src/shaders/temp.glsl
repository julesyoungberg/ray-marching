...
vec3 calcNormal(vec3 pos) {
    ...
    return normalize(k.xyy * map(pos + k.xyy * h, trash) +
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec3 cam_pos = vec3(3.0 * cos(0.1 * 0.125 * freq) * sin(0.1 * 0.5 * freq),
                        sin(0.1 * freq), 2.0 * cos(0.1 * 0.5 * freq));
    ...
    vec2 p = (2.0 * fragCoord - iResolution.xy) / iResolution.y;
            ...
            vec3 col =
                mix(vec3(0.001), vec3(0.05), clamp(exp(2.0 * p.y), 0.0, 1.0));

            vec3 ro = cam_pos;
            vec3 rd =
                normalize(p.x * h * cam_uu + p.y * h * cam_vv + cam_ww - ro);
            ...
            */
            ...
            if (t > 0.0) {
                vec3 base_col1 =
                    mix(vec3(0.906, 0.929, 0.922), vec3(0.373, 0.18, 0.18),
                        abs(sin(0.1 * iTime)));
                vec3 base_col2 =
                    mix(vec3(0, 0.227, 0.42), vec3(0.165, 0.125, 0.165),
                        abs(sin(0.1 * iTime)));
                vec3 base_col3 =
                    mix(vec3(0.259, 0.765, 0.969), vec3(0.545, 0.255, 0.212),
                        abs(sin(0.1 * iTime)));
