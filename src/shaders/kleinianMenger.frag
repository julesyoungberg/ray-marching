...
float distFromNearest(in vec3 pos) {
    vec3 orbitTrap;
    return distFromNearest(pos, orbitTrap);
}
...
vec4 scene(in vec3 rayOrigin, in vec3 rayDir) {
    ...
    vec3 orbitTrap;
    float dist = distFromNearest(ro, orbitTrap) * 0.8;
    ...
    for (int i = 0; i < NUM_STEPS; i++) {
        ...
        dist = distFromNearest(p, orbitTrap);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        dist = smoothstep(0.0, 0.005, distFromNearest(p));
